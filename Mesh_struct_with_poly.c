...
void calc_Parabolic_Crank(M *m)
{
    if(m->size==3)
    {
        ...
        //remove the bottom line, its a flag 
        print_mesh(*m);
    }
    if(m->size==4)
    {
    ...
    {
        //remove flag
        printf("Entered the 6-value crank calculations\n");
        double d1,d2,d3,d4;
        for(int i=(m->timesteps)-2;i>=0;i--)
        {
            d1=(m->b[0][i]+m->b[0][i+1]+m->b[2][i+1]);
        ...
        //remove flag
        printf("%d",1);
        printf("exited the 6-value crank calculations\n");
    }
    if(m->size>6)
    {
...
{
    int ch;
    printf("Enter 0 for inputing each border value and 1 for input of step values\n");
    scanf("%d",&ch);
    if(ch==0)
    {
