...
void startWith(NODE first,char ch)
{
    if(first==NULL)
    {
    ...
    printf("Elements of DLL starting with %c are:\n",ch);
    NODE cur=first;
    int count=0;
    while(cur!=NULL)
    {
        if((cur->info).Ename[0]==ch)
        {
            show(cur->info);
            count++;
        }
...
void update(NODE first)
{
    ...
    while (cur!=NULL)
    {
        if((cur->info).Eid==tag)
        {
            printf("Replacing %d-%s\nEnter Substituting Name: ",tag,(cur->info).Ename);
            scanf(" %[^\n]",(cur->info).Ename);
            found=1;
        }
        cur=cur->rlink;
    ...
    if(found==0)
    {
        printf("No element with matching tag found");
    }
}
...
{
    printf("Enter 1 to go right and 0 to go left and -1 to exit\n");
    int ch;
    NODE cur=first;
    if(cur==NULL)
    {
